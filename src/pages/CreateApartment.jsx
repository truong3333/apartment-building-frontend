...
export default function CreateApartment(){
  ...
  async function submit(e){
    ...
    setErrors({})
    const err = {}
    if(!roomNumber) err.roomNumber = 'Số phòng là bắt buộc'
    if(!area || Number(area) <= 0) err.area = 'Diên tích phải lớn hơn 0'
    if(Object.keys(err).length){ setErrors(err); return }
    setLoading(true)
    try{
      const payload = { roomNumber, area: Number(area), address }
      const res = await api.post('/api/v1/apartment', payload)
      setMsg(res.data.result || 'Thêm căn hộ thành công')
  ...
  return (
    <div className="bg-white rounded shadow p-6 max-w-lg mx-auto">
      <h3 className="text-lg font-semibold mb-4">Thêm căn hộ</h3>
      <form onSubmit={submit} className="space-y-3">
        ...
        <div>
          <label className="block text-sm">Diện tích (m²)</label>
